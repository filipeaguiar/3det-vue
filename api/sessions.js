...
export default async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        try {
            ...
            if (data.ganchos_personagens) data.ganchos_personagens.forEach(g => stmts.push({ sql: 'INSERT INTO session_ganchos_personagens (id, session_id, personagem_id, personagem_name, description) VALUES (?, ?, ?, ?, ?)', args: [uuidv4(), sessionId, g.personagem_id || null, g.personagem_name || '', g.description] }));
            if (data.locais_interessantes) {
                data.locais_interessantes.forEach(l => {
                    const localId = uuidv4();
                    stmts.push({ sql: 'INSERT INTO session_locais_interessantes (id, session_id, name, description) VALUES (?, ?, ?, ?)', args: [localId, sessionId, l.name, l.description || ''] });
            ...
            if (data.npcs_importantes) data.npcs_importantes.forEach(n => stmts.push({ sql: 'INSERT INTO session_npcs_importantes (id, session_id, npc_id, name, role, notes) VALUES (?, ?, ?, ?, ?, ?)', args: [uuidv4(), sessionId, n.npc_id || null, n.name || '', n.role || '', n.notes || ''] }));
            if (data.encontros_desafios) data.encontros_desafios.forEach(e => stmts.push({ sql: 'INSERT INTO session_encontros_desafios (id, session_id, name, description, mecanica) VALUES (?, ?, ?, ?, ?)', args: [uuidv4(), sessionId, e.name, e.description || '', e.mecanica || ''] }));
            if (data.segredos_rumores) data.segredos_rumores.forEach(s => stmts.push({ sql: 'INSERT INTO session_segredos_rumores (id, session_id, description, revealed) VALUES (?, ?, ?, ?)', args: [uuidv4(), sessionId, s.description, s.revealed ? 1 : 0] }));
            if (data.tesouros_recompensas) data.tesouros_recompensas.forEach(t => stmts.push({ sql: 'INSERT INTO session_tesouros_recompensas (id, session_id, description, claimed) VALUES (?, ?, ?, ?)', args: [uuidv4(), sessionId, t.description, t.claimed ? 1 : 0] }));
